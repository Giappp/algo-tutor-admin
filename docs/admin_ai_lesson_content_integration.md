...
LLM chỉ trả về Markdown thuần. Backend validate Markdown và build `LessonRequestDTO` phù hợp với loại lesson hiện tại. Admin không thể dùng API này để đổi `type`, `title`, `difficulty` hoặc `displayOrder`.
...
| `QUIZ` | Parse Markdown checkbox thành `QuizLessonRequestDTO.questions`; giữ nguyên passing score và time limit hiện tại |
...
| `content` | `LessonRequestDTO` | Bản nháp lesson có thể chỉnh sửa và gửi sang API update |
...
`content` là discriminated union theo trường `type`. Đây là DTO được backend build sau khi nhận Markdown từ LLM, không phải JSON do LLM trả trực tiếp.
...
interface QuizLessonDraft extends LessonBase {
  type: "QUIZ";
...
```ts
function openDraftEditor(draft: LessonDraft) {
  switch (draft.type) {
    case "THEORY":
      return openTheoryEditor(draft);
    case "CODING":
      return openCodingEditor(draft);
    case "QUIZ":
      return openQuizEditor(draft);
  }
}
```
...
4. FE nhận `data.content`, đưa vào form editor dưới dạng bản nháp.
...
Với `QUIZ`, `PUT /lessons/{lessonId}` chỉ cập nhật metadata như `title`, `difficulty`, `passingScore` và `timeLimitMinutes`; endpoint này hiện không đồng bộ danh sách `questions`.

FE cần lưu từng câu hỏi bằng API questions:
...
```ts
const quizDraft = generated.content as QuizLessonDraft;

await api.put(`/lessons/${generated.lessonId}`, quizDraft);
